		...
		class DaylightMap {
			constructor(svg, date, options = {}) {
				if (
					...
					throw new Error(
						"Unmet dependency (requires d3.js, jQuery, SunCalc)"
					);
				}
				if (!svg) {
					throw new TypeError(
				...
				this.WORLD_PATHS_URL =
					"https://s3-us-west-2.amazonaws.com/s.cdpn.io/215059/world-110m.json";
			...
			getSunriseSunsetLatitude(lng, northSun) {
				...
				while (lat !== endLat) {
					if (
						this.isDaylight(SunCalc.getPosition(this.currDate, lat, lng))
					) {
						return lat;
					}
					lat += delta;
			...
			getAllSunriseSunsetCoords(northSun) {
				...
				while (lng < 180) {
					coords.push([
						this.getSunriseSunsetLatitude(lng, northSun),
						lng
					]);
			...
			getCityOpacity(coord) {
				if (
					SunCalc.getPosition(this.currDate, coord[0], coord[1])
					.altitude > 0
				) {
					return 0;
				}
